...
load('GvMoverNYC_48station_800m_5_uniform.mat', 'stationArray')
...
MAXREPEAT = 5;
% k = 3;
numWorkersRatio = 1:0.1:2
costList_GLA_G = zeros(size(numWorkersRatio));
costList_GLA_M = zeros(size(numWorkersRatio));
timeList_GLA_G = zeros(size(numWorkersRatio));
timeList_GLA_M = zeros(size(numWorkersRatio));
totalWorkerNumber_GLA = zeros(size(numWorkersRatio));

costList_1GA_G = zeros(size(numWorkersRatio));
costList_1GA_M = zeros(size(numWorkersRatio));
timeList_1GA_G = zeros(size(numWorkersRatio));
timeList_1GA_M = zeros(size(numWorkersRatio));
totalWorkerNumber_1GA = zeros(size(numWorkersRatio));

costList_2GA_G = zeros(size(numWorkersRatio));
costList_2GA_M = zeros(size(numWorkersRatio));
timeList_2GA_G = zeros(size(numWorkersRatio));
timeList_2GA_M = zeros(size(numWorkersRatio));
totalWorkerNumber_2GA = zeros(size(numWorkersRatio));

index = 1;
for i = numWorkersRatio
    % i stands for number of stations  
    i
    [positive_gla_mul, negative_gla_mul] = GLA_multiRound(round(stationArray(:,4)*0.15),stationArray(:,4),-stationArray(:,5:28)+stationArray(:,29:52));    
    [positive_1ga_mul, negative_1ga_mul, feasible] = kGA_multiRound(round(stationArray(:,4)*0.15),stationArray(:,4),-stationArray(:,5:28)+stationArray(:,29:52),1);
    ...
    for gla_i = 1:size(positive_gla_mul,2)
        numWorkers = round(abs(sum(positive_gla_mul(:,gla_i)))*i);
        if numWorkers==0
            continue;
        end
...
%         negative = negative(negative(:,3)~=0,:);
    ...
    for ga1_i = 1:size(positive_1ga_mul,2)
        numWorkers = round(abs(sum(positive_1ga_mul(:,ga1_i)))*i);
        if numWorkers==0
            continue;
        end
...
%         negative = negative(negative(:,3)~=0,:);
    ...
    index = index+1;
    %disp(i);
    save('GvMoverNYC_48station_800m_5_change_worker_revised.mat');
end
end
