%% Extended Two Round Matching algorithm (number of users/workers could be more than positive/negative demands)
%WARING: Positive and negative has different meaning in paper!
%positive in the program means the negative in the paper
function [assignment, cost] = extendedTwoRoundMatching(users, positive, negative)
...
%% Extended two round matching (need to make sure size of extendedPositive and extendedNegative are the same.)
...
%   Construct user-station matchin graph
    nextLine = 1;
    edges_user_stations = zeros(m*n, 3);
    for (i = 1 : m)
        for (j = 1 : n)
            edges_user_stations(nextLine, :) = [i, j+m, ...    % means user i rent bike at station j return at stationPair(j)
                ...
                + 0.33*distance(extendedPositive(j,:), extendedNegative(stationPair(j)-n,:))/180*pi*6371 ...
...
%% Generate Output
    ...
    for i = 1 : m
        if (user_station_Pair(i)~=-1)
            ...
            assignment(assignmentIndex, :) = [i, correspondingPositiveStation, correspondingNegativeStation];
