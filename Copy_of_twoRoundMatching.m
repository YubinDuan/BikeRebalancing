...
%   Construct user-station matchin graph
    nextLine = 1;
    edges_user_stations = zeros(n^2, 3);
    for (i = 1 : n)
        for (j = 1 : n)
            edges_user_stations(nextLine, :) = [i, j+n, ...    % means user i rent bike at station j return at stationPair(j)
...
%% Generate Output
    ...
    cost = 0;
    for i = 1 : n
        correspondingPositiveStation = user_station_Pair(i)-n;
